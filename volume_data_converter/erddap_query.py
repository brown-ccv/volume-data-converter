...
configuration_file_path = os.path.join(
    Path(__file__).absolute().parent, "config", "erddap_configuration.json"
)
...
def erddap_query(
    ...
    if os.path.isdir(output_file_path):
        typer.echo(
            f"Ouput folder path is a directory. File-name auto assigned to: {erddap_connection_object['dataset_id']}.nc"
        )
        output_file_path = os.path.join(
            output_file_path, erddap_connection_object["dataset_id"] + ".nc"
        )
    else:
        ...
        if not os.path.isdir(folder_path):
            raise (
                f" Could not find output path: {output_file_path}. Check first argument"
            )
    ...
    erddap_obj.dataset_id = erddap_connection_object["dataset_id"]
